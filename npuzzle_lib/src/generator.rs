	...
	G: Grid + Hash + Eq + Clone,
{
	...
	pub fn generate_random(&mut self, iterations: usize) -> Result<()> {
		self.generate_solution()?;
		self.shuffle(iterations)
	}

	/// should create the "default state" aka solution
	pub fn generate_solution(&mut self) -> Result<()> {
	...
	///
	fn shuffle(&mut self, iterations: usize) -> Result<()> {
		if iterations == 0 {
			return Ok(());
		}
...
mod test {
	...
	#[test]
	fn test_grid2d() {
		let g = Grid2D::with_capacity(5, 5);
		let mut gen = Generator::new(g, PuzzleType::Snail);
		// gen.generate_solution();
		assert!(gen.generate_random(10).is_ok());
